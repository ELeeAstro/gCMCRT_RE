...
contains
  ...
  attributes(global) subroutine pp_grey_kernel(Nph)
    implicit none
  ...
  end subroutine pp_grey_kernel


end module gCMCRT_grey
...
subroutine grey()
  ...
  integer :: u_nml
  integer :: Nph, Nit, last_int_fac, nlay
  real(dp) :: Tint, k_lw, p_top, p_bot, T_IC, grav_const, Rd_gas, lw_a, lw_g
  ...
  namelist /grey_nml/ Nph, Nit, last_int_fac, nlay, &
    & Tint, k_lw, p_top, p_bot, T_IC, grav_const, Rd_gas, lw_a, lw_g

  !! Read input variables from namelist
  open(newunit=u_nml, file='gCMCRT_RE.nml', status='old', action='read')
  read(u_nml, nml=grey_nml)
  close(u_nml)
  ...
  ! Constant grey opacity - single band
  nb = 1
  ...
  !! Output IC
  open(newunit=u0,file='results/Tp_grey_1D_pp.txt',action='readwrite')
  write(u0,*) Nit, nlay
  write(u0,*) 0
  do i = 1, nlay
  ...
  do n = 1, Nit
    ...
    call pp_grey_kernel<<<blocks, threads>>>(Nph_d)
    ...
    OLR = OLR_d
    print*, 'OLR: ', (OLR/sb_c)**(1.0_dp/4.0_dp), 'Tint: ', Tint

  end do


  open(newunit=u3,file='results/estim_pp_grey_1D_pp.txt',action='readwrite')
  do i = 1, nlay
